  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-gradient-to-b from-slate-900 to-slate-950 p-4 sm:p-5 shadow-xl">
      <div className="flex flex-col sm:flex-row sm:items-baseline justify-between gap-2">
        ...
        {typeof processing_time === 'number' && <span className="text-slate-400 text-sm sm:text-base">Processed in {processing_time.toFixed(2)}s</span>}
      </div>
      {predictions.length === 0 ? (
        <p className="text-slate-400 mt-2 text-sm sm:text-base">No predictions available.</p>
      ) : (
